...
const GraphCanvas = forwardRef<GraphCanvasRef, GraphCanvasProps>(
  ({ data, onSelect, selectedId, selectedLink, searchQuery, theme }, ref) => {
    ...
    const colors = {
      ...
      linkSentiment: {
        ...
        sameAuthor: isDark ? "#64748b" : "#94a3b8",
        // sameAuthor: isDark ? '#475569' : '#cbd5e1'
      },
    };
    ...
    useEffect(() => {
      ...
      const explicitLinks = data.links.map((l) => ({ ...l }));

      // --- Runtime Detection of Same-Author Links ---
      const authorMap = new Map<string, Node[]>();
      clonedNodes.forEach((n) => {
        if (!authorMap.has(n.author)) authorMap.set(n.author, []);
        authorMap.get(n.author)!.push(n);
      });

      const implicitLinks: Link[] = [];

      authorMap.forEach((books) => {
        if (books.length < 2) return;
        // Sort by year to create a chronological chain
        books.sort((a, b) => (a.year || 0) - (b.year || 0));

        for (let i = 0; i < books.length - 1; i++) {
          const source = books[i];
          const target = books[i + 1];

          // Check if ANY explicit link already exists between these two (direction agnostic)
          const exists = explicitLinks.some(
            (l) =>
          ...
          if (!exists) {
            implicitLinks.push({
              ...
              target: target.id,
              isSameAuthor: true,
              // No sentiment or quote required for implicit links
            } as Link);
          }
        }
      });
      ...
      linksRef.current = [...explicitLinks, ...implicitLinks];
      ...
      linksRef.current.forEach((l) => {
        ...
        nodeCounts[sourceId as number] =
          (nodeCounts[sourceId as number] || 0) + 1;
        nodeCounts[targetId as number] =
          (nodeCounts[targetId as number] || 0) + 1;
        inDegreeCounts[targetId as number] =
          (inDegreeCounts[targetId as number] || 0) + 1;
      });
      ...
      const getFontSize = (d: Node) =>
        Math.min(20, Math.max(12, 11 + (d.connectionCount || 0)));

      // Simulation setup
      const simulation = d3
        .forceSimulation<Node, Link>(nodesRef.current)
        .force(
          "link",
          d3
      ...
      const link = g
        ...
        .attr("cursor", "pointer")
        .attr("stroke-dasharray", (d) => (d.isSameAuthor ? "5,5" : null))
        .on("click", (event, d) => {
    ...
    useEffect(() => {
      ...
      const circles = gRef.current.selectAll<SVGCircleElement, Node>(
        ".nodes circle"
      );
      ...
      circles
        ...
        .duration(300)
        .attr("fill", (d) =>
          isNodeActive(d) ? colors.nodeSelected : colors.node
        )
        .style("filter", (d) => {
        ...
        .attr("opacity", (d) => {
          ...
          if (selectedId) {
            ...
            const isNeighbor = linksRef.current.some(
              (l) =>
                ((l.source as Node).id === selectedId &&
                  (l.target as Node).id === d.id) ||
                ((l.target as Node).id === selectedId &&
      ...
      texts
        ...
        .duration(300)
        .attr("fill", (d) =>
          isNodeActive(d) ? colors.textSelected : colors.text
        )
      ...
      links
        ...
        .style(
          "filter",
          isDark ? "drop-shadow(0 0 2px rgba(148, 163, 184, 0.5))" : null
        ) // Add glow to links
        .attr("opacity", (d) => {
      ...
      const selectionChanged =
        prev.id !== selectedId || prev.link !== selectedLink;
